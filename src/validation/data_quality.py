...
class DataQualityValidator:
    ...
    def validate(self, df: DataFrame) -> Dict[str, Any]:
        ...
        report = {"timestamp": F.current_timestamp(), "total_records": df.count(), "checks": {}}
        ...
        try:
            ...
            failed_checks = [
                name for name, result in report["checks"].items() if not result.get("passed", True)
            ]
            ...
            if failed_checks and self._has_critical_failures(report["checks"]):
                raise DataQualityError(f"Critical data quality checks failed: {failed_checks}")
    ...
    def _check_nulls(self, df: DataFrame) -> Dict[str, Any]:
        ...
        for column in df.columns:
            ...
            null_counts[column] = {"count": null_count, "percentage": round(null_percentage, 2)}

        # Check if null percentage exceeds threshold
        max_null_pct = self.config.max_null_percentage
        violations = {
            col: data for col, data in null_counts.items() if data["percentage"] > max_null_pct
        }
    ...
    def _check_negative_values(self, df: DataFrame) -> Dict[str, Any]:
        ...
        result = {"passed": passed, "violations": violations, "checked_columns": columns_to_check}
